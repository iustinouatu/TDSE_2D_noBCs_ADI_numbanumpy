...
def main():
    ...
    # Real Time Propagation (RTP)
        ...
        for timestep in range(0, gl.N_timesteps):
            psi = computations_real_1timestep(psi, temp, timestep, Z)
...
@njit
def computations_real_1timestep(psi, tem, timeste, Z):
# # Propagation of psi in the laser field:

    # Do one sweep across all i-lines   
    # | | ... | | ... | |
    for p in range(1, gl.N_z_divs):
        tem[p, :] = solve_for_1_fixed_i_line_Voft_ON_trisolver(p, timeste, psi, Z)
        if p % 200 == 0:
    ...
    # ---------------> u = j = 1
    for u in range(1, gl.N_epsilon_divs):
        psi[:, u, timeste+1] = solve_for_1_fixed_j_line_Voft_ON_trisolver(u, timeste, tem, Z) # returned from solve_for_1_fixed_j_line() is shape (N_z_divs+1, )
        if u % 20 == 0:
...
@njit
def solve_for_1_fixed_i_line_Voft_ON_trisolver(i, timestep, psi, Z): # i represents the i-line we are solving for by calling this function: i=1 is the second vertical line
    ...
    for con in range(1, gl.N_epsilon_divs): #
        j = con
        d[con] = 1 + (1j*gl.delta_time/2) * (  (1/(2*gl.lambd**2*(j*gl.delta_epsilon)**(2*gl.lambd))) * (2*j**2 - (gl.lambd-0.5)**2) + 0.5*Voft(i, j, np.float64(timestep+0.5), gl.m, Z)    ) # diag 
    ...
    for row in range(1, gl.N_epsilon_divs):
        ...
        b[row] = psi[i, row, timestep] - (1j*gl.delta_time/2) * (  (-0.5*second_der_wrt_z)  + 0.5*Voft(i, j, np.float64(timestep+0.5), gl.m, Z)*psi[i, row, timestep] )
        
    return tridiag_solver_from3vectors(d, c, a, b) # returned is shape (gl.N_epsilon_divs, ) (same as b.shape)

@njit
def solve_for_1_fixed_j_line_Voft_ON_trisolver(j, timestep, temp, Z):
    ...
    for con in range(1, gl.N_z_divs): # last element it does work on is ab[1][gl.N_z_divs - 1]:
        i = con
        d[con] = 1 + (1j*gl.delta_time/2) * ( (1/gl.delta_z**2) + 0.5*Voft(i, j, np.float64(timestep+0.5), gl.m, Z) )
